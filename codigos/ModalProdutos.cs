...
{
    public class ModalProdutos : Form
    {
        ...
        private void InitUI()
        {
            ...
            // ── TableLayoutPanel: 3 linhas ─────────────────────────────────────
            // Row 0 = header verde (50px)
            // Row 1 = filtros (44px)
            // Row 2 = grid (*)
            // Row 3 = rodapé (52px)
            ...
            _grid.Columns.Add(new DataGridViewTextBoxColumn { Name="colId",    HeaderText="ID",      FillWeight=9  });
            _grid.Columns.Add(new DataGridViewTextBoxColumn { Name="colNome",  HeaderText="Nome",    FillWeight=44 });
            _grid.Columns.Add(new DataGridViewTextBoxColumn { Name="colValor", HeaderText="Valor",   FillWeight=18 });
            _grid.Columns.Add(new DataGridViewTextBoxColumn { Name="colEstq",  HeaderText="Estoque", FillWeight=13 });
        ...
        private void CarregarGrid()
        {
            ...
            foreach (var p in lista)
            {
                int idx = _grid.Rows.Add(
                    p.Id, p.Nome, $"R$ {p.Valor:F2}",
                    p.Estoque, p.Ativo ? "✓ Ativo" : "✗ Inativo");
                if (!p.Ativo)
                {
        ...
        private void Selecionar()
        {
            ...
            {
                MessageBox.Show("Este produto esta inativo.",
                    "Produto Inativo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ProdutoSelecionado = DatabaseHelper.BuscarProdutoPorId(id);
